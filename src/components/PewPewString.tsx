...
interface PewPewStringProps extends React.DetailedHTMLProps<React.IframeHTMLAttributes<HTMLIFrameElement>, HTMLIFrameElement> {
    level: string;
}
...
function PewPewString(props: PewPewStringProps) {
    const [levelString, setLevel] = useState<Uint8Array>(new Uint8Array(0));
    useEffect(() => {
        const zip = new JSZip();
